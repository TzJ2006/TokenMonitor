...
pub fn is_menu_bar_dark() -> bool {
    ...
    let Some(mtm) = MainThreadMarker::new() else {
        return true; // Not on main thread, assume dark
    };

    let app = NSApplication::sharedApplication(mtm);
    let names = objc2_foundation::NSArray::from_retained_slice(&[
...
fn has_utilization(config: &TrayConfig, claude_util: Option<f64>, codex_util: Option<f64>) -> bool {
    match config.bar_display.as_str() {
        "both" => claude_util.is_some() || codex_util.is_some(),
        "single" => {
...
pub fn render_tray_icon(
    ...
    if config.bar_display == "off" || !has_utilization(config, claude_util, codex_util) {
...
mod tests {
    ...
    fn make_config(bar_display: &str) -> TrayConfig {
        TrayConfig {
            bar_display: bar_display.to_string(),
    ...
    fn off_returns_original_icon_with_template() {
        ...
        let (buf, w, h, tmpl) = render_tray_icon(&icon, &make_config("off"), None, None, true);
        assert_eq!(w, 44);
        assert_eq!(h, 44);
    ...
    fn no_utilization_returns_template() {
        let icon = vec![0u8; (44 * 44 * 4) as usize];
        let (_, w, _, tmpl) = render_tray_icon(&icon, &make_config("both"), None, None, true);
        assert_eq!(w, 44);
    ...
    fn dark_bar_icon_is_white() {
        ...
        let (buf, w, _, _) =
            render_tray_icon(&icon, &make_config("both"), Some(50.0), Some(50.0), true);
    ...
    fn light_bar_icon_is_black() {
        ...
        icon[idx + 3] = 200;
        let (buf, w, _, _) =
            render_tray_icon(&icon, &make_config("both"), Some(50.0), Some(50.0), false);
        let dst_idx = (10 * w as usize + 10) * 4;
