...
/// Read a previously exported document from `path` and import it. The frontend
/// obtains `path` from the native file picker rather than an HTML
/// `<input type="file">`: on Linux, WebKitGTK maps the input's `accept`
/// extensions to MIME types and `.jsonl` has no registered MIME, so jsonl
/// exports are greyed out and unselectable. A native dialog with extension
/// filters (handled by the OS picker) selects them reliably on every platform.
#[tauri::command]
pub async fn import_usage_file(
    app: AppHandle,
    state: State<'_, AppState>,
    path: String,
) -> Result<ImportResult, String> {
    ...
    import_usage_data(app, state, contents).await
}
