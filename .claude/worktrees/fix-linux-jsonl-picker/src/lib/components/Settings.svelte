<script lang="ts">
  ...
  async function triggerImport() {
    ...
    try {
      ...
      // resulting blur so the main window doesn't auto-hide while it's open.
      await invoke("suppress_next_auto_hide");
      // Native Open dialog with extension filters. An HTML <input type="file">
      // accept filter hides `.jsonl` exports in WebKitGTK on Linux, so the
      // native picker is used on every platform instead.
      const selected = await open({
        multiple: false,
        directory: false,
        filters: [{ name: "Usage Export", extensions: ["json", "jsonl"] }],
      });
    ...
    ioMessage = null;
    try {
      const result = await importUsageFile(path);
