<script lang="ts">
  import type { UsageProviderLogoKind } from "../providerMetadata.js";

  interface Props {
    kind: UsageProviderLogoKind;
    size?: number;
    class?: string;
  }

  let { kind, size = 14, class: className = "" }: Props = $props();
</script>

{#if kind === "all"}
  <svg class={className} width={size} height={size} viewBox="0 0 24 24" fill="currentColor" fill-rule="evenodd">
...
{:else if kind === "cursor"}
  <svg class={className} width={size} height={size} viewBox="0 0 512 512" fill="currentColor">
